...
func (g *PromotionGate) Run(ctx context.Context, input Input, confirmation string) (Summary, []Finding, error) {
	if g.Logger == nil {
		g.Logger = log.Default()
	}
	if strings.TrimSpace(g.ConfirmationPhrase) == "" {
		return Summary{}, nil, fmt.Errorf("confirmation phrase is required")
	}
	if confirmation != g.ConfirmationPhrase {
		...
		return Summary{Block: 1}, []Finding{block}, nil
	}
	...
	runner := NewRunner(g.Checks, g.Logger)
...
func missingChecks(required []string, checksList []PreflightCheck) []string {
...
func flattenResults(results []Result) []Finding {
	findings := []Finding{}
